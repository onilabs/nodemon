...
function exec(nodemonOptions, execMap) {
  ...
  if (options.exec.charAt(0) !== '"') {
    ...
    // executable has been specified in quotes, to protect spaces (e.g. "C:/Program Files/xxx")
    var matches = /^"([^"]+)"(.*)$/.exec(options.exec);
    if (matches) {
      options.exec = matches[1];
      options.execArgs = matches[2].split(' ');
    }
  }
